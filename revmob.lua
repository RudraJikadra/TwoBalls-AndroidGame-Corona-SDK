package.preload['json']=(function(...)local s={}local e=string
local r=math
local c=table
local i=error
local f=tonumber
...
return e
end)package.preload['revmob_context']=(function(...)local o=require('json')local i=require('revmob_about')local e=require('revmob_log')local r=require('revmob_device')local a=require('revmob_cache')local n
...
return nil
end,sdkPayload=function(e)local n={name=i["NAME"],version=i["VERSION"]}if e then
n.testing_mode=e
end
return n
...
local o=string.sub(n.social.birthday,1,4)local r=string.sub(n.social.birthday,6,7)local n=string.sub(n.social.birthday,9,10)local t=os.time{year=o+t,month=r,day=n,hour=0,min=0}local n=os.time()if not(n<t)then
e.social=i
end
end
end
...
return e
end,payloadAsJsonString=function(e,t)return o.encode(n.payload(e,t))end,printEnvironmentInformation=function(o,t,l,s)e.debug("==============================================")e.debug(n.payloadAsJsonString(t,l))local d=system.getInfo("iosAdvertisingIdentifier")local u=system.getInfo("iosIdentifierForVendor")local c=system.getInfo("iosAdvertisingTrackingEnabled")local n=nil
local l=nil
if o~=nil then
n=tostring(o[REVMOB_ID_IOS])l=tostring(o[REVMOB_ID_ANDROID])end
...
if l~=nil then e.release("Publisher App ID for Android: "..l)end
e.release("Device name: "..system.getInfo("name"))e.release("Model name: "..system.getInfo("model"))e.release("Device ID: "..system.getInfo("deviceID"))e.release('IDFA (iOS only): '..tostring(d))e.release('IDFV (iOS only): '..tostring(u))e.release('Limit ad tracking (iOS only): '..tostring(c))e.release("Environment: "..system.getInfo("environment"))e.release("Platform name: "..system.getInfo("platformName"))e.release("Platform version: "..system.getInfo("platformVersion"))e.release("Corona version: "..system.getInfo("version"))e.release("Corona build: "..system.getInfo("build"))e.release("Architecture: "..system.getInfo("architectureInfo"))e.release("Locale-Country: "..system.getPreference("locale","country"))e.release("Locale-Language: "..system.getPreference("locale","language"))e.release("Timeout: "..tostring(s).."s")e.release("Corona Simulator: "..tostring(r.isSimulator()))e.release("iOS Simulator: "..tostring(r.isXcodeSimulator()))if t~=nil then e.release("Installed in this device: "..tostring(a.isInstallRegistered(t)))end
...
end)package.preload['revmob_client']=(function(...)local g=require('json')local p=require('socket.http')local l=require("ltn12")local e=require('revmob_about')local n=require('revmob_log')local t=require('revmob_messages')local r=require('revmob_events')local f=require('revmob_context')local w=require('revmob_cache')local i=require('revmob_device')local a=require('revmob_utils')REVMOB_ID_IOS='iPhone OS'REVMOB_ID_ANDROID='Android'local e
local L=30
local c='https://ios.revmob.com'local h='https://android.revmob.com'local S="app"local y=false
local D=true
local I=10
local u=true
local d=0
local o=nil
local function s()if"Android"==system.getInfo("platformName")then
return h
else
return c
end
end
local function v(e)if e.phase=="ended"then
if o~=nil then
o:removeSelf()o=nil
end
end
...
local function m(t,o,i,n)if n==nil then
local n key="fetch_"..o
local e=e.serverEndPoints[key]if e~=nil then
return e
end
return s().."/api/v4/mobile_apps/"..t.."/"..i.."/fetch.json"else
local o="fetch_"..o.."_with_placement"local e=e.serverEndPoints[o]if e~=nil then
e=string.gsub(e,"PLACEMENT_ID",n)return e
end
return s().."/api/v4/mobile_apps/"..t.."/placements/"..n.."/"..i.."/fetch.json"end
end
local function c(n)local e=e.serverEndPoints['install']if e~=nil then
return e
else
return s().."/api/v4/mobile_apps/"..n.."/install.json"end
end
local function _(e)return s().."/api/v4/mobile_apps/"..e.."/sessions.json"end
local function s(o,t)local i=f.payloadAsJsonString(e.appId,e.testMode)if e.testMode~=nil then
...
local function h(e)return e and string.len(e)==24
end
...
if e==nil then
e=n[REVMOB_ID_IOS]if not h(e)then
e=n[REVMOB_ID_ANDROID]i.setSimulatorAndroid()else
i.setSimulatorIOS()end
end
return e
end
local function b()local n=function(t)n.debugTable(t)local t=t.status or t.statusCode
if(t==200)then
w.saveInstallWasRegistered(e.appId)n.release("Install received.")if listener~=nil then
...
s(c(e.appId),n)end
local function i(l,a,i,o)local i=f(i)if e.sessionStarted then
if i~=nil then
n.info("Ad registered with Placement ID "..i)end
s(m(e.appId,l,a,i),o)else
...
local c
local function m(e)local t={}local n=""local i,t,n=p.request{method="POST",url=e,source=l.source.string(n),headers={["Content-Length"]=tostring(#n),["Content-Type"]="application/json"},sink=l.sink.table(t),}return c(e,t,n)end
c=function(n,t,i)if(type(t)=='number'and t>=300 and t<=308)then
...
e={TEST_WITH_ADS="with_ads",TEST_WITHOUT_ADS="without_ads",TEST_DISABLED=nil,appId=nil,appIds=nil,sessionStarted=false,testMode=nil,listenersRegistered=false,serverEndPoints={},startSession=function(i)local t=f(i)if h(t)then
...
if(i==200)then
local t,n=pcall(g.decode,t.response)if(t~=nil and n~=nil)then
...
s(_(t),e)n.info("Session started for App Id: "..t)return t
else
...
end,registerInstall=function()if w.isInstallRegistered(e.appId)then
n.info("Install already registered in this device")else
b()end
end,fetchFullscreen=function(e,n)i('fullscreen','fullscreens',e,n)end,fetchBanner=function(n,e)i('banner','banners',n,e)end,fetchLink=function(e,n)i('link','anchors',e,n)end,fetchPopup=function(n,e)i('pop_up','pop_ups',n,e)end,reportImpression=function(e)if e~=nil then
n.info("Reporting impression")s(e,nil)else
n.debug("No impression url")end
end,theFetchSucceed=function(a,o,l)n.debugTable(o)local e=o.status or o.statusCode
...
if l~=nil then l({type=r.AD_NOT_RECEIVED,ad=a,reason=i})end
return false,nil
end
if e==302 or e==303 then
return true,nil
end
local i,e=pcall(g.decode,o.response)if(not i or e==nil)then
n.release("Reason: "..t.UNKNOWN_REASON..tostring(i).." / "..tostring(e))if l~=nil then l({type=r.AD_NOT_RECEIVED,ad=a,reason=reason})end
return false,e
end
return i,e
...
e:addEventListener("touch",v)o:insert(e)local e=native.newWebView(0,e.height+display.screenOriginY,display.contentWidth,display.contentHeight-display.screenOriginY*2)e:request(n)o:insert(e)end,getLink=function(n,e)for t,e in ipairs(e)do
if e.rel==n then
return e.href
end
end
...
e=""end
local i,e,t=p.request{method="POST",url=n,source=l.source.string(e),headers={["Content-Length"]=tostring(#e),["Content-Type"]="application/json"},sink=l.sink.table(t),}return c(n,e,t)end,getMarketURLWithoutFollow=function(e)u=false
local e=m(e)u=true
return e
...
e.setTimeoutInSeconds(L)return e
end)package.preload['revmob_fullscreen_web']=(function(...)local o=require('revmob_log')local a=require('revmob_messages')local r=require('revmob_events')local n=require('revmob_client')require('revmob_sound_player')local i="fullscreen"local t
...
end,load=function(e,l)e.networkListener=function(t)local t,l=n.theFetchSucceed(i,t,e.listener)if t then
local t=l['fullscreen']['links']e.impressionUrl=n.getLink('impressions',t)e.clickUrl=n.getLink('clicks',t)e.htmlUrl=n.getLink('html',t)o.release("Fullscreen loaded")if e.listener~=nil then e.listener({type=r.AD_RECEIVED,ad=i})end
if e.autoshow then
e:show()end
end
end
n.fetchFullscreen(l,e.networkListener)end,isLoaded=function(e)return(e.htmlUrl~=nil and e.clickUrl~=nil)or(e.dspHtml~=nil)end,hide=function(e)if not e:isLoaded()then e.autoshow=false end
...
e.clickListener=function(l)if string.sub(l.url,-string.len("#close"))=="#close"then
...
if string.sub(l.url,-string.len("#click"))=="#click"then
if e.changeOrientationListener then
Runtime:removeEventListener("orientation",e.changeOrientationListener)end
...
if l.errorCode then
o.release("Error: "..tostring(l.errorMessage))end
return true
end
...
e.clickSoundURL=i
local t=t['fullscreen']['links']local i=n.getLink('impressions',t)local r=n.getLink('clicks',t)local s=n.getLink('html',t)local u=n.getLink('image',t)local c=n.getLink('close_button',t)local a=n.getLink('dsp_url',t)local n=n.getLink('dsp_html',t)if n~=nil then
...
e.view:show()end
elseif a~=nil then
o.debug("DSP url fullscreen")e.view=l.new(e.params)e.view.htmlUrl=a
e.view.impressionUrl=i
e.view.clickUrl=r
e.view.autoshow=e.autoshow
if e.autoshow==true then
e.view:show()end
elseif s~=nil then
o.debug("Rich fullscreen")e.view=l.new(e.params)e.view.htmlUrl=s
e.view.impressionUrl=i
e.view.clickUrl=r
e.view.autoshow=e.autoshow
if e.autoshow==true then
...
end)package.preload['revmob_banner_web']=(function(...)local o=require('revmob_log')local s=require('revmob_messages')local l=require('revmob_events')local t=require('revmob_utils')local d=require('revmob_device')local n=require('revmob_client')require('revmob_sound_player')local r="banner"local i
...
end,load=function(e,a)e.networkListener=function(t)local i,t=n.theFetchSucceed(r,t,e.listener)if i then
local t=t['banners'][1]['links']e.impressionUrl=n.getLink('impressions',t)e.clickUrl=n.getLink('clicks',t)e.htmlUrl=n.getLink('html',t)o.release("Banner loaded")if e.listener~=nil then e.listener({type=l.AD_RECEIVED,ad=r})end
e:configWebView()if e.autoshow then
e:show()end
end
end
n.fetchBanner(a,e.networkListener)end,configWebView=function(e)e.clickListener=function(i)if string.sub(i.url,-string.len("#click"))=="#click"then
if e.listener~=nil then e.listener({type=l.AD_CLICKED,ad=r})end
local t=nil
if e.followRedirect==true then
t=n.getMarketURL(e.clickUrl)else
t=n.getMarketURLWithoutFollow(e.clickUrl)end
o.info(s.OPEN_MARKET)if t then
if e.openInside==true and e.appOrSite=="site"then
n.openWebView(t)else
system.openURL(t)end
end
...
e.webView:addEventListener("tap",e.webView)e.webView:addEventListener("touch",e.webView)end,isLoaded=function(e)return(e.htmlUrl~=nil and e.clickUrl~=nil)or(e.dspHtml~=nil)end,show=function(e)local a=(t.Screen.width()>640)and 640 or t.Screen.width()local i=(d.isIPad()and 100 or 50*(t.Screen.bottom()-t.Screen.top())/display.contentHeight)local d=(t.Screen.left()+a/2)local t=(t.Screen.bottom()-i/2)e:setPosition(e.x or d,e.y or t)e:setDimension(e.width or a,e.height or i)if not e:isLoaded()then
if e.autoshow==true then
o.info(s.AD_NOT_LOADED)end
e.autoshow=true
return
end
if e.webView~=nil then
timer.performWithDelay(1,function()if e.listener~=nil then e.listener({type=l.AD_DISPLAYED,ad=r})end
...
return i
end)package.preload['revmob_banner_static']=(function(...)local i=require('revmob_log')local a=require('revmob_messages')local l=require('revmob_events')local t=require('revmob_utils')local d=require('revmob_device')local n=require('revmob_client')require('revmob_sound_player')local r="banner"local o
...
end,setDimension=function(e,t,n,i)if e.image~=nil then
...
return o
end)package.preload['revmob_banner']=(function(...)local i=require('revmob_log')local t=require('revmob_client')local u=require('revmob_banner_static')local l=require('revmob_banner_web')local c=require('revmob_events')local o="banner"RevMobBanner={params=nil,view=nil,placementIds=nil,listener=nil,autoshow=true,width=nil,height=nil,x=nil,y=nil,rotation=0,new=function(n)local e=n or{}setmetatable(e,RevMobBanner)e.params=n
return e
end,load=function(e)local n=function(n)local o,n=t.theFetchSucceed(o,n,e.listener)if o then
local o=t.getDefaultValue("app_or_site")if(n['banners']['app_or_site']~=nil)then
o=n['banners']['app_or_site']end
e.appOrSite=o
local o=t.getDefaultValue("open_inside")if(n['banners']['open_inside']~=nil)then
o=n['banners']['open_inside']end
e.openInside=o
local o=t.getDefaultValue("follow_redirect")if(n['banners']['follow_redirect']~=nil)then
o=n['banners']['follow_redirect']end
e.followRedirect=o
local o=""if(n['banners']['sound']~=nil)then
...
e.clickSoundURL=o
local n=n['banners'][1]['links']local o=t.getLink('impressions',n)local r=t.getLink('clicks',n)local d=t.getLink('image',n)local a=t.getLink('html',n)local s=t.getLink('dsp_url',n)local n=t.getLink('dsp_html',n)if n~=nil then
i.debug("DSP hmtl banner")if e.params==nil then e.params={}end
...
e.params["listener"]=RevMobBannerViewListener
i.debug("DSP url banner")e.view=l.new(e.params)e.view.htmlUrl=s
e.view.impressionUrl=o
e.view.clickUrl=r
e.view.autoshow=e.autoshow
e.view:configWebView()if e.autoshow==true then
...
if e.params==nil then e.params={}end
e.params["listener"]=RevMobBannerViewListener
i.debug("Rich banner")e.view=l.new(e.params)e.view.htmlUrl=a
e.view.impressionUrl=o
e.view.clickUrl=r
e.view.autoshow=e.autoshow
e.view:configWebView()if e.autoshow==true then
...
i.debug("Static banner")if e.params==nil then e.params={}end
e.params["listener"]=RevMobBannerViewListener
e.view=u.new(e.params)e.view.imageUrl=d
e.view.impressionUrl=o
e.view.clickUrl=r
e.view.autoshow=e.autoshow
...
RevMobBannerViewListener=function(e)if e.type==c.AD_CLICKED and e.ad=="banner"then
RevMobBanner:load()end
end
...
e.x=n or e.view.x
e.y=t or e.view.y
end,setDimension=function(e,t,i,n)if e.view~=nil then e.view:setDimension(t,i,n)end
...
r.info("Link opened")r.info(a.OPEN_MARKET)if e.openInside==true and e.appOrSite=="site"then
n.openWebView(t)else
system.openURL(t)end
else
...
return n end)package.preload['revmob_sound_player']=(function(...)local o=require("audio")local i=require("revmob_download_manager")local e=require('revmob_log')SoundPlayer={}SoundPlayer={playSoundFromURL=function(e)if(e~=nil and string.len(e)>0)then
local r="/"local t=nil
local n=string.find(e,r,1)t=n
while n do
n=string.find(e,r,n+1)if n then
t=n
end
end
local n=string.sub(e,t+1)i.download(e,n)local e=o.play(o.loadSound(i.fileName,i.params.response.baseDirectory))end
